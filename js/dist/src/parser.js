      ...
      if (m && !sawTransition) {
        ...
        pushPending(
          ...
          phrase.includes("destroy")
            ? model_1.Actions.Destroy
            : phrase.includes("updated")
            ? model_1.Actions.Modify
            : model_1.Actions.Create,
        );
        continue;
      }
    }
 ...
 */
function resolveUnit(unit, sawTransition) {
  if (
...
function parse(raw) {
  ...
  const resolution =
    units.some((u) => u.resolution === model_1.RESOLUTION.FAILURE)
