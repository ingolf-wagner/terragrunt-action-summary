...
// Resolution badges
const RESOLUTION_BADGE: Record<Resolution, string> = {
  success: "✅",
  failure: "❌",
  unknown: "❓",
};
...
export function splitModulePrefix(address: string): [string | null, string] {
...
export function renderUnit(unit: Unit): string {
  ...
  if (unit.counts !== null) {
    const total = countTotal(unit.counts);
    if (total === 0) {
      summary += "**No changes.**\n";
  ...
  if (unit.resources.length > 0) {
    ...
    const groups: [(string | null), Unit["resources"]][] = [];
    ...
    for (const [modulePath, group] of groups) {
      ...
      for (const resource of group) {
        ...
        const time = resource.durationSeconds !== null
          ...
          : "";
        items.push(
          `| ${cell} | ${ICONS[resource.action]} | ${
            STATE_BADGE[resource.state]
          } | ${time} |`,
        );
      }
    }
...
export function renderMarkdown(units: Unit[], resolution: Resolution): string {
  ...
  let countsKind: Unit["countsKind"] = null;
 ...
 */
export function render(run: TerraformRun): string {
  const nothingRecognized = run.units.every(
    (u) => u.resolution === RESOLUTION.UNKNOWN,
  );
